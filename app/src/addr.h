...
/// Return the number of items in the multisig address view
zxerr_t addr_multisig_getNumItems(uint8_t *num_items);

/// Gets an specific item from the multisig address view (including paging)
zxerr_t addr_multisig_getItem(int8_t displayIdx, char *outKey, uint16_t outKeyLen, char *outValue, uint16_t outValueLen,
                              uint8_t pageIdx, uint8_t *pageCount);
