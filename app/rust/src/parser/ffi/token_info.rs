...
pub fn get_token_info<'a, T, U>(contract_address: T, contract_name: U) -> Option<TokenInfo<'a>>
where
    T: AsRef<[u8]>,
    U: AsRef<[u8]>,
{
    ...
    // Copy input slices and ensure null termination
    let addr_len = core::cmp::min(addr_bytes.len(), CONTRACT_ADDR_STR_MAX_LEN);
    addr_null_term[..addr_len].copy_from_slice(&addr_bytes[..addr_len]);

    let name_len = core::cmp::min(name_bytes.len(), CONTRACT_ADDR_STR_MAX_LEN);
    name_null_term[..name_len].copy_from_slice(&name_bytes[..name_len]);
