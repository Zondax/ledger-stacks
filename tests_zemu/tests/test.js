...
const models = [
  ['S', { model:'nanos', prefix: 'S', app_path: APP_PATH_S}],
  ['X', { model: 'nanox', prefix: 'X', app_path: APP_PATH_X}]
];
...
describe("Basic checks", function() {
  ...
  test.each(models)('can start and stop container (%s)', async function(_, {model, prefix, app_path}) {
  ...
  test.each(models)('app version (%s)', async function(_, {model, prefix, app_path}) {
    const sim = new Zemu(app_path);
    try {
      await sim.start({model, ...simOptions});
  ...
  test.each(models)('get address (%s)', async function(_, {model, prefix, app_path}) {
    const sim = new Zemu(app_path);
    try {
      await sim.start({model, ...simOptions});
      const app = new BlockstackApp(sim.getTransport());
  ...
  test.each(models)('show address (%s)', async function(_, {model, prefix, app_path}) {
    const sim = new Zemu(app_path);
    try {
      await sim.start({model, ...simOptions});
      const app = new BlockstackApp(sim.getTransport());
      ...
      await sim.compareSnapshotsAndAccept(".", `${prefix.toLowerCase()}-show-address`, model === 'nanos' ? 3 : 3);
  ...
  test.each(models)('test signature (%s)', async function(_, {model, prefix, app_path}) {
    const sim = new Zemu(app_path);
    ...
    try {
      ...
      await sim.compareSnapshotsAndAccept(".", `${prefix.toLowerCase()}-signatureTest`, model === 'nanos' ? 9 : 8);
