...
describe("Basic checks", function () {
  ...
  test.each(models)(`app version`, async function (m) {
    const sim = new Zemu(m.path);
    try {
      await sim.start({...defaultOptions, model: m.name})
  ...
  test.each(models)(`show address`, async function (m) {
    ...
    try {
      ...
      await sim.compareSnapshotsAndAccept(".", `${m.prefix.toLowerCase()}-show-address`, m.name === 'nanos' ? 2 : 2);
  ...
  test.each(models)(`sign`, async function (m) {
    ...
    try {
      ...
      await sim.compareSnapshotsAndAccept(".", `${m.prefix.toLowerCase()}-sign`, m.name === 'nanos' ? 8 : 7);
  ...
  test.each(models)(`multisig`, async function (m) {
    ...
    try {
      ...
      const sigHashPreSign = makeSigHashPreSign(
        ...
        unsignedTx.auth.authType,
        unsignedTx.auth.spendingCondition?.fee,
        unsignedTx.auth.spendingCondition?.nonce).toString();
      ...
      await sim.compareSnapshotsAndAccept(".", `${m.prefix.toLowerCase()}-multisigTest`, m.name === 'nanos' ? 8 : 7);
      ...
      // @ts-ignore
      const presig_hash = [
        Buffer.from(signer0.sigHash, 'hex'),
        Buffer.alloc(1, unsignedTx.auth.authType),
        feeBytes,
        nonceBytes
      ];
